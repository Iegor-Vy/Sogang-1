...
I hope you all are doing well and ready for yet another **exciting session** of Mathematical Methods for International Commerce.
...
# Week 3/2: Indices and Logarithms, Exponential and Natural Log Functions
---

# Why economists care

* Growth and inflation use exponentials

* Interest with compounding is exponential

* Logs turn multiplication into addition

* Logs help interpret percent changes and elasticities
---

# Indices: meaning and negative powers

* $a^n$ as repeated multiplication
...
# Laws of indices (only the ones we use constantly)
...
Mini example: $2^3\cdot2^2=2^5$
---
# Log definition (inverse idea)
...
Examples: $\log_2 8=3$, $\log_{10}1000=3$
...
* $\log(x^c)=c\log x$
  ...
  Example: $\log_2(4\cdot 8)=5$
---

# Solving exponential equations with logs

Method: take logs both sides

Example: $2^x=16 \Rightarrow x=\log_2 16=4$
---
...
* Continuous growth: $A=A_0 e^{rt}$

Continuous growth means that growth is happening at every instant in time.

### Example (continuous compounding)
$$
A=1000e^{0.05\cdot 3}=1000e^{0.15}\approx 1161.83
$$
...
Interpretation: continuous compounding increases the amount faster than simple interest.

---
# Logs in economics: constant-elasticity demand
Assume $Q=aP^b$. 
...
Example: $Q=200P^{-0.5}\Rightarrow E=-0.5$ (inelastic since $|E|<1$)

Constant elasticity means that the percentage change in quantity demanded resulting from a percentage change in price remains constant along the demand curve.
---

# What you must be able to do in class

* simplify using index laws

* evaluate basic logs

* solve equations like $e^{2x}=10$ using $\ln$

* interpret elasticity sign and magnitude
...
# Indices and Logarithms, Exponential and Natural Log Functions
...
5. **Use Logs:** Solve $e^{2x} = 10$, hint: take natural logs. $\ln(10) \approx 2.3026; \approx 1.1513$ 
...
If function form is $Q = aP^b$, taking logarithms:
...
$$ E = -0.5 $$ (inelastic demand) means quantity responds less than proportionally to price changes.
...
- (Mar 24) Percentages (3.1), Compound Interest (3.2)    
...
renderthis::to_pdf("W-3_2_ME.html")

getwd()
setwd("C:\\Users\\vyshn\\OneDrive - kdis.ac.kr\\Documents\\GitHub\\Sogang\\2026\\Spring\\Mathematical Economics\\Week 3_2")
